...
def validate_races_headers(df_races: pd.DataFrame) -> None:
    """Validate that the ingested races data contains exactly the expected columns."""
    ...
    extra_headers = [header for header in df_races.columns if header not in required_headers]

    if len(df_races.columns) != len(required_headers):
        raise ValueError(
            f"Invalid races headers. Expected {len(required_headers)} columns, found {len(df_races.columns)}"
        )

    if missing_headers or extra_headers:
        raise ValueError(
            f"Invalid races headers. Missing: {missing_headers}; Extra: {extra_headers}"
        )
...
def validate_results_headers(df_results: pd.DataFrame) -> None:
    """Validate that the ingested results data contains exactly the expected columns."""
    ...
    extra_headers = [header for header in df_results.columns if header not in required_headers]
    
    if len(df_results.columns) != len(required_headers):
        raise ValueError(
            f"Invalid results headers. Expected {len(required_headers)} columns, found {len(df_results.columns)}"
        )
